...
function Voucher() {
        ...
        await VoucherService.getVoucherByFilter(filterVoucher)
            .then(response => {
                ...
                const formattedData = response.data.map((voucher, index) => ({
                    ...voucher,
                    key: index + 1,
                    startTime: FormatDate(voucher.startTime),
                    endTime: FormatDate(voucher.endTime)
                }));
    ...
    const columns = [
...

        {
            ...
            width: '10%',
            render: (status) => (
                status === 1 ? <Tag style={{ borderRadius: '4px', fontWeight: '450', padding: '0 4px ' }} color="#108ee9">Hoạt động</Tag>
                    : <Tag style={{ borderRadius: '4px', fontWeight: '450', padding: '0 4px ' }} color="#f50">Hết hạn</Tag>
            ),
        },
            ...
            <Col span={24}>
                ...
                >
                    <Form
                        ...
                        initialValues={{
                            ...reacord,
                            ...(reacord?.startTime && { startTime: dayjs(reacord.startTime, "HH:mm:ss - DD/MM/YYYY") }),
                            ...(reacord?.endTime && { endTime: dayjs(reacord.endTime, "HH:mm:ss - DD/MM/YYYY") }),
                        }}
                                    ...
                                    rules={[
                                        { required: true, message: 'Vui lòng nhập mã giảm giá!' },
                                        {
                                            validator: (_, value) => {
                                                ...
                                                const trimmedValue = value.trim().toLowerCase(); // Chuyển về chữ thường và loại bỏ dấu cách ở đầu và cuối
                                                const isDuplicate = vouchers.some(
                                                    (voucher) => voucher.code.trim().toLowerCase() === trimmedValue && voucher.id !== reacord.id
                                                );
                                                if (isDuplicate) {
                                    ...
                                    rules={[
                                        ...
                                        ({ getFieldValue }) => ({
                                            validator(_, endTime) {
                                                ...
                                                if (dayjs(endTime).isBefore(currentDate, 'day')) {
