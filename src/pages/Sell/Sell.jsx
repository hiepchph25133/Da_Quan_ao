...
export default function Sell() {
    ...
    const handleUpdateOrder = async (values) => {
        console.log('Sending values to backend:', values); // In ra values để kiểm tra
        try {
    ...
    const handlePlaceOrder = async () => {
        try {
            ...
            values.statusName = tabStates[orderId] ? 'Chờ xác nhận' : 'Hoàn thành';
            values.orderTotal = calculateTotal();
            values.transportFee = calculateTransportFee();

            // Lấy tên của địa chỉ từ danh sách cities, districts, và wards
            const selectedCity = cities.find(city => city.idProvince === values.city);
            const selectedDistrict = districts.find(district => district.idDistrict === values.district);
            const selectedWard = wards.find(ward => ward.idCommune === values.ward);

            if (selectedCity) values.city = selectedCity.name;
            if (selectedDistrict) values.district = selectedDistrict.name;
            if (selectedWard) values.ward = selectedWard.name;

            // Kiểm tra xem có thông tin user không, nếu không thì là khách lẻ
            if (!order.user) {
                values.recipientName = values.recipientName || 'Khách lẻ';
                values.phoneNumber = values.phoneNumber || 'Không có';
                values.addressDetail = values.addressDetail || 'Không có';
            }

            console.log('Order values:', values); // Debugging output
                            ...
                            <Row>
                                <Col span={12} style={{ paddingRight: '15px' }}>
                                    <Form.Item
                                        ...
                                        name="city"
                                        rules={tabStates[tabKey] ? [{ required: true, message: 'Vui lòng chọn tỉnh/thành phố!' }] : []}
                                ...
                                <Col span={12} style={{ paddingRight: '15px' }}>
                                    <Form.Item
                                        label="Quận/Huyện:"
                                        name="district"
                                        rules={tabStates[tabKey] ? [{ required: true, message: 'Vui lòng chọn quận/huyện!' }] : []}
                            ...
                            <Row>
                                <Col span={12} style={{ paddingRight: '15px' }}>
                                    <Form.Item
                                        label="Phường/Xã:"
                                        name="ward"
                                        rules={tabStates[tabKey] ? [{ required: true, message: 'Vui lòng chọn phường/xã!' }] : []}
                                ...
                                <Col span={12} style={{ paddingRight: '15px' }}>
                                    <Form.Item
                                        ...
                                        rules={tabStates[tabKey] ? [{ required: true, message: 'Vui lòng nhập địa chỉ cụ thể!' }] : []}
                                    >
                                        <Input
...
const ModalVoucher = ({ hideModal, isModal, order, findOrderById, calculateTotalAmount }) => {
    ...
    const fetchVoucher = async () => {
        try {
            const response = await VoucherService.findAllVoucherByDeletedTrue();
    ...
    useEffect(() => {
        const cartVoucherId = order.voucher ? order.voucher.code : null;
    ...
    const handleOkInput = async () => {
        setErrorText('');
        if (!inputVoucherCode) {
            setErrorText('Vui lòng nhập mã giảm giá!');
            return;
        }
        const selectedVoucherData = vouchers.find(
            (voucher) => voucher.code === inputVoucherCode
        );
        ...
        if (selectedVoucherData) {
            try {
                await OrderService.updateOrderVoucher(order.id, inputVoucherCode);
                findOrderById();
                notification.success({
                    message: 'Thông báo',
            ...
            setErrorText('Không có mã giảm giá này!');
        }
    };
    ...
    const handleOk = async () => {
        ...
        try {
            await OrderService.updateOrderVoucher(order.id, selectedVoucher);
            findOrderById();
            hideModal();
        } catch (err) {
            console.log(err);
    ...
    return (
        ...
        >
            <Row style={{ marginTop: '20px', padding: '15px', backgroundColor: '#ece9e9' }}>
                ...
                <Col span={6}>
                    <Button disabled={!inputVoucherCode} style={{ float: 'right' }} type='primary' onClick={handleOkInput}>
                        Áp dụng
                    </Button>
                </Col>
            </Row>
            ...
            <div style={{
                ...
                {vouchers.map((item, index) => (
                    <Row key={index} style={{ marginTop: '7px', padding: '7px', border: '1px solid #cdcdcd' }}>
                        ...
                        <Col span={2}>
                            <Radio
                                ...
                                onChange={() => setSelectedVoucher(item.code)}
